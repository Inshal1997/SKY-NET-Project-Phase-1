...
{
    public partial class AMD_Stats_Controls : UserControl
    {
       ...
       // SqlConnection connection = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["db"].ConnectionString);
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                connection.Open();
                query = "select PCISlot,name from AMD_GpuData";
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            //AMD data fetch
          /*  aMD_Stats_DataSet.Clear();
            this.aMD_GpuDataTableAdapter.Fill(this.aMD_Stats_DataSet.AMD_GpuData);
        ...
        {
            // Hashratecharts.Update();
            // Hashratecharts.Visible = false;
            // Hashratecharts.Hide();
        ...
        {
            AMD_Stats_Controls s = new AMD_Stats_Controls();
            s.Form1_Load(s, e);
            ...
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("select time,Hashrate from Stats_AMD_GPUsData where PCISlot = @PCI_Index", con);
                cmd.Parameters.AddWithValue("@PCI_Index", ind);
                SqlDataReader RDR = cmd.ExecuteReader();
                while (RDR.Read())
                {
                    Console.WriteLine(Convert.ToDouble(RDR["Hashrate"]));
                ...
                Hashratecharts.Series = new SeriesCollection
            {
                ...
                // con.ConnectionString = ConfigurationManager.ConnectionStrings["conDB"].ConnectionString;
                con.Open();
                cmd = new SqlCommand("select time,tempminer from Stats_AMD_GPUsData where PCISlot = @PCI_Index", con);
                cmd.Parameters.AddWithValue("@PCI_Index", ind);
                ...
                temp_chart.Series = new SeriesCollection
            {
                ...
                con.Open();
                cmd = new SqlCommand("select time,power from Stats_AMD_GPUsData where PCISlot = @PCI_Index", con);
                cmd.Parameters.AddWithValue("@PCI_Index", ind);
